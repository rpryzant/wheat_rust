...
class CNN():
    def __init__(self, config):
        ...
        l1_n = config.l1_n
        self.conv1_1 = conv_relu_batch(self.x, l1_n, filter_size,1, name="conv1_1")
        conv1_1_d = tf.nn.dropout(self.conv1_1, self.keep_prob)
        conv1_2 = conv_relu_batch(conv1_1_d, l1_n, filter_size,2, name="conv1_2")
        conv1_2_d = tf.nn.dropout(conv1_2, self.keep_prob)

        l2_n = config.l2_n
        conv2_1 = conv_relu_batch(conv1_2_d, l2_n, filter_size,1, name="conv2_1")
        conv2_1_d = tf.nn.dropout(conv2_1, self.keep_prob)
        conv2_2 = conv_relu_batch(conv2_1_d, l2_n, filter_size,2, name="conv2_2")
        conv2_2_d = tf.nn.dropout(conv2_2, self.keep_prob)

        l3_n = config.l3_n
        conv3_1 = conv_relu_batch(conv1_2_d, l3_n, filter_size,1, name="conv3_1")
        conv3_1_d = tf.nn.dropout(conv3_1, self.keep_prob)
        conv3_2= conv_relu_batch(conv3_1_d, l3_n, filter_size,1, name="conv3_2")
        conv3_2_d = tf.nn.dropout(conv3_2, self.keep_prob)
        conv3_3 = conv_relu_batch(conv3_2_d, l3_n, filter_size,2, name="conv3_3")
        conv3_3_d = tf.nn.dropout(conv3_3, self.keep_prob)
        pool3 = pool2d(conv3_3_d, 2, name="pool3")

        # conv4_1 = conv_relu_batch(pool3, 512, 3, 1, name="conv4_1")
        # conv4_1_d = tf.nn.dropout(conv4_1, self.keep_prob)
        # conv4_2 = conv_relu_batch(conv4_1_d, 512, 3, 1, name="conv4_2")
        # conv4_2_d = tf.nn.dropout(conv4_2, self.keep_prob)
        # conv4_3 = conv_relu_batch(conv4_2_d, 512, 3, 1, name="conv4_3")
        ...
        self.fc6 = dense(flattened, config.dense, name="fc6")
